...
- 首先CIP和VIP肯定在同一个网段(只是能路由到),否则,CIP根本无法发送请求报文;如果RIP(RS的物理网卡的IP地址)和CIP不在同一网段,由于RS必须通过RIP所在的接口送出报文,那么这种情况下客户端根本无法接受到响应报文;要想让客户端能收到响应报文,那么在RS外面还得再加一个路由设备,把RS的RIP的网关都指向这个路由设备;然后打开路由转发功能,把路由器的另外一个接口设置为和VIP在同一个网段.这样就可以了.
-

...
- 启动mysqld:`service mysqld start`
    ...
    ```
-  二进制同拥包会一次性帮我们把mysq和mysql都装好,我们只需要把/usr/local/mysql/bin/添加到/etc/profiled/mysql.sh中即可
    ```
...
- 除了ab,还有其他的压力测试工具:
    ...
    - siege
    - webbench
- 让我们自己编译安装的http服务器启用ssl功能(https), 编辑/etc/httpd.conf,启用 LoadModule ssl_module module_ssl.so; 然后编辑/etc/httpd/extra/httpd-ssl.conf;主要就是指定证书的路径.
##### /etc/httpd/extra/中其他的配置文件    
- httpd-mpm.conf: 定义没中mpm工作的参数
- 

...
###### 客户端排除:
  ...
  [root@vm3 rsync]# ls
  1  10  2  3  4  5  6  7  8  9  a  b  c  d  e
  [root@vm3 rsync]# rm -rf *
  [root@vm3 rsync]# ls

  // 在客户端先看下我们要同步目录中有哪些文件
  [root@vm1 ~]# cd /tmp/rsync
  [root@vm1 rsync]# ls
  1  10  2  3  4  5  6  7  8  9  a  b  c  d  e
  // 假设我们推送的时候要排除a文件
  [root@vm1 rsync]# rsync -avz --exclude=a /tmp/rsync/ rsync_backup@192.168.1.33::oldboy --password-file=/etc/rsync.password  //a没有被通不过去
  sending incremental file list
  ./
  1
  10
...
- 客户端命令排除多个文件我们用{a,b}来把a和b排除掉
  ```
  [root@vm1 rsync]# rsync -avz --exclude={a,b} /tmp/rsync/ rsync://rsync_backup@192.168.1.33/oldboy --password-file=/etc/rsync.password
  sending incremental file list
  ./
  1
  10
...
- --exclude-from=文件列表,我们可以把列表中的文件都排除了
  ```
  [root@vm1 rsync]# seq 10 >paichu.log
  [root@vm1 rsync]# cat paichu.log
  1
  2
  3
  4
  ...
  9
  10
  [root@vm1 rsync]# rsync -avz --exclude-from=paichu.log /tmp/rsync/ rsync_backup@192.168.1.33::oldboy --password-file=/etc/rsync.password  //表示把paichu.log中的文件从同步的文件中排除掉
  sending incremental file list
  ./
  a
  b
